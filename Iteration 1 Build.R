...
names(Out_of_school_Aus) <- gsub("_", " ", names(Out_of_school_Aus))
#Out_of_school_Aus$Count_of_Males <- gsub("_", " ", Out_of_school_Aus$Count_of_Males)
...
#Out_of_school_Aus_melt$value <- format(Out_of_school_Aus_melt$value, big.mark = ",")
...
ggplot(data = Children_connect, aes(x = Year, y = percent)) +
  geom_bar(stat = "identity", alpha = 0.75, fill="darkblue",width = 0.6)  +
  geom_bar(aes(x = Year + .2, y= percent - .5),stat = "identity", alpha=.2,width = 0.5) +
  ylim(0,100) +
  theme(plot.title = element_text(size = 18)) +
  #scale_fill_viridis(discrete = F) +
  geom_label(label= Children_connect$percent, show_guide = F,color = "black", fill="white",size=3) +
  # geom_text( fontface = "bold", vjust = 1.5,
  #          position = position_dodge(.9), size = 4) +
  labs(x = "\n Year", y = "Percentage of Connectedness", title = "Figure 4: Children connectedness with their school percentage in Victoria. ",
       caption = "Source : AURIN") +
...
p <- ggplot(data = Retention_Rate, aes(x = Year, y = Total_percent)) +
  geom_bar(stat = "identity", alpha = 0.75, fill="darkblue",width = 0.6)  +
  geom_bar(aes(x = Year + .2, y= Total_percent - .5),stat = "identity", alpha=.2,width = 0.5) +
  ylim(0,100) +
  theme(plot.title = element_text(size = 18)) +
  #scale_fill_viridis(discrete = F) +
  ...
  # geom_text( fontface = "bold", vjust = 1.5,
  #          position = position_dodge(.9), size = 4) +
  labs(x = "\n Year", y = "Percentage Retention Rate", title = "Figure 5: Retention Rate of Children in Australia.",
       caption = "Source : Australian Bureau of Statistics") +
  scale_x_discrete(limits = unique(Retention_Rate$Year)) 


#source_text <- "Source: AURIN"
#p <- p +
#  annotate("text", x = Inf, y = Inf, label = source_text, hjust = 0, vjust = 0, size = 4)

print(p)

output_text <- "Output: Your Output"
grid.text(output_text, x = unit(1, "npc") - unit(1, "cm"), y = unit(1, "npc") - unit(0.5, "cm"), just = "right")
